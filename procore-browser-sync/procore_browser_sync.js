...
async function commandSyncAuto(args) {
  ...
  const result = await syncToCommandCenter(tasks, args["complete-missing"] === true, auth);
...
async function syncToCommandCenter(tasks, completeMissing, auth) {
  if (!auth) {
    return applySourceTasksToD1({
      source: "procore",
      tasks,
      replaceProjectIds: completeMissing ? [...new Set(tasks.map(task => String(task.projectId)))] : []
    });
  }
